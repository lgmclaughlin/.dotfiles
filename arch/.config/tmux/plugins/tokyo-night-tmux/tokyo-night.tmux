...
window_title="#($SCRIPTS_PATH/window-title.sh #{pane_id} #{pane_current_path})"
...
# Focused
tmux set -g window-status-current-format "$RESET#[fg=${THEME[green]},bg=${THEME[bblack]}] \
#{?#{==:#{pane_current_command},ssh},󰣀 ,$active_terminal_icon $window_space}\
#[fg=${THEME[foreground]},bold,nodim]$window_number $window_title\
#[nobold]#{?window_zoomed_flag, $zoom_number, $custom_pane}#{?window_last_flag, , }"

# Unfocused
tmux set -g window-status-format "$RESET#[fg=${THEME[foreground]}] \
#{?#{==:#{pane_current_command},ssh},󰣀 ,$terminal_icon $window_space}${RESET}$window_number $window_title\
#[nobold,dim]#{?window_zoomed_flag, $zoom_number, $custom_pane}#[fg=${THEME[yellow]}]#{?window_last_flag,󰁯  , }"
